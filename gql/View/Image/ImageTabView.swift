...
//  gql
//
//  Created by Shaw Young on 2022/2/28.
//
...
struct ImageTabView: View {
    @EnvironmentObject var imageViewModel: ImageTabViewModel
    @GestureState var imageOffset: CGSize = .zero
    @Namespace var ns
    var body: some View {
        ZStack {
            imageViewModel.showingImages ? Color.black.opacity(imageViewModel.bgOpacity).edgesIgnoringSafeArea(.all) : Color.white.edgesIgnoringSafeArea(.all)
            TabView(selection: $imageViewModel.selectedImageIndex) {
                ForEach(0...imageViewModel.images.count-1, id: \.self) { idx in
                    WebImage(url: URL(string: imageViewModel.images[idx])!)
                        .resizable()
                        .aspectRatio(contentMode: .fit)
                        .matchedGeometryEffect(id: "img\(idx)", in: ns)
                        .offset(x: imageViewModel.imageScale == 1 ? 0 : imageViewModel.offset.width,y: imageViewModel.offset.height)
                        .scaleEffect(imageViewModel.selectedImageIndex == idx ?
                                     (imageViewModel.imageScale > 1 ? imageViewModel.imageScale : 1) : 1)
                        .gesture(
                            MagnificationGesture()
                                .onChanged({ value in
                                    imageViewModel.imageScale = value
                                })
                                .onEnded({ _ in
                                    withAnimation(.spring()) {
                                        imageViewModel.imageScale = 1
                                    }
                                })
                                .simultaneously(with: TapGesture(count: 2)).onEnded({ _ in
                                    withAnimation {
                                        imageViewModel.offset = imageViewModel.imageScale > 1 ? .zero : imageViewModel.offset
                                        imageViewModel.imageScale = imageViewModel.imageScale > 1 ? 1 : 4
                                    }
                                })
                        )
                }
            ...
            .gesture(
                DragGesture()
                    .updating($imageOffset, body: { value, state, _ in
                        ...
                        imageViewModel.onChange(offset: imageOffset)
                    })
                    .onEnded(imageViewModel.onEnded(value:))
            )
//            .onTapGesture {
//                withAnimation {
//                    imageViewModel.showingImages.toggle()
//                }
//            }
            .tabViewStyle(PageTabViewStyle(indexDisplayMode: imageViewModel.images.count == 1 ? .never : .always))
        }
...
//    static var previews: some View {
//        SDGridImageView(images: Array(
//            arrayLiteral:
//                "https://dev-1306842204.cos.ap-guangzhou.myqcloud.com//postImages/0x1915d/a3063ffb-5c5e-ea5b-eddc-a2e4aa0b2473/微信图片_20220302111048.jpg"
