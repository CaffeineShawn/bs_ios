...
struct SDGridImageView: View {
    @ObservedObject var imageTabViewModel: ImageTabViewModel
    ...
    init(images: [String]) {
        ...
        _imageTabViewModel = ObservedObject(wrappedValue: ImageTabViewModel(images: images))
    }
    ...
    var body: some View {
                ...
                Group {
                    ...
                    case 1:
                        SDWebImageView(urlString: images.first!, width: 260, height: 260)
                            .onTapGesture {
                                withAnimation {
                                    imageTabViewModel.toggleShowingImageView(selectedImageIndex: 0)
                                }
                            }
                    case 2...3:
                        LazyVGrid(columns: triGridItems, spacing: 7) {
                            ForEach(0...images.count-1, id: \.self) { idx in
                                SDWebImageView(
                                    urlString: images[idx],
                                    width: (UIScreen.main.bounds.width-84)/3-14/3,
                                    height: (UIScreen.main.bounds.width-84)/3
                                )
                                    .frame(width: abs(proxy.size.width/3-14/3))
                                    .onTapGesture {
                                        withAnimation {
                                            imageTabViewModel.toggleShowingImageView(selectedImageIndex: idx)
                                        }
                                    }
                            }
                        }
                                ...
                                SDWebImageView(
                                    ...
                                    .onTapGesture {
                                        if images[2] != "" {
                                            ...
                                            withAnimation {
                                                imageTabViewModel.toggleShowingImageView(selectedImageIndex: (idx < 2 ? idx : idx-1))
                                            }
                                        }
        ...
        .fullScreenCover(isPresented: $imageTabViewModel.showingImages) {
            ZStack {
                ImageTabView()
                    .frame(width: UIScreen.main.bounds.width)

                    .environmentObject(imageTabViewModel)
            }
        }
    }
}

struct MoreImagesIndicatorView: View {
    let count: Int
    var body: some View {
        Text("+\(count-6)")
...
//struct CoverView: UIViewRepresentable {
